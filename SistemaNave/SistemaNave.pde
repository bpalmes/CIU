...
//Cordenadas del vector de la camara
...
void actualizarCamara(){
  if (w){
    navePos.add(PVector.mult(dir, 10));
  }
  
  if (s){
    navePos.sub(PVector.mult(dir, 10));
  }
  ...
  if (a){
    ...
    producto.normalize();
    navePos.sub(PVector.mult(producto, 10));
  }
  ...
  if (d){
    ...
    PVector.cross(dir, up, producto);
    producto.normalize();
    navePos.add(PVector.mult(producto, 10));
  }
